...
class LineFlickerMaterialProperty {
	...
	getValue(time: any, result: any) {
		...
		result.color = (Cesium as any).Property.getValueOrDefault(this._color, time, this.color, result.color);
		result.speed = (Cesium as any).Property.getValueOrDefault(this._speed, time, this.speed, result.speed);
		return result;
	}
	...
	equals(other: any) {
		return (
			...
			(other instanceof LineFlickerMaterialProperty &&
				(Cesium as any).Property.equals(this._color, other._color) &&
				(Cesium as any).Property.equals(this._speed, other._speed))
		);
	}
}
...
export function lineFlickerMaterial(viewer: Cesium.Viewer, color: any, speed: number) {
	...
	(Cesium as any).Material._materialCache.addMaterial("LineFlickerMaterialProperty", {
		fabric: {
